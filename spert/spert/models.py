...
class SpERT(BertPreTrainedModel):
    ...
    def _classify_entities(self, encodings, h, entity_masks, size_embeddings):
        ...
        # use CLS embedding directly from first token
        entity_ctx = h[:, 0, :]   # [batch_size, hidden_dim]
        
        # reshape for concatenation
        entity_ctx = entity_ctx.unsqueeze(1)
        
        # repeat across entity candidates
        entity_ctx = entity_ctx.repeat(
            1,
            entity_spans_pool.shape[1],
            1
        )
        
        # combine context + span + size embeddings
